...
## ADR-0034 — Correct the Shorts format multiplier (0.1 → 0.5)

**Status:** Accepted (Day 14)

**Context:** A brand-match review surfaced sponsored-cost figures that looked wrong: a Shorts-heavy Fitness channel at 338K median views priced at ₹32K while a long-form DIY channel at 104K median priced at ₹73K. Investigation confirmed the pricing model itself was sound (reach-first: median views × niche sponsored-CPM band × format multiplier, subscriber-floored, ₹50L-capped) and that `_est_cost` (the old AdSense-CPM helper) is dead code — the live path is `attach_est_earnings` → `integration_cost_range`. The real fault was the Shorts format multiplier: `_SHORTS_FORMAT = 0.1` (10% of long-form), which contradicted the file's own docstring ("Shorts pay ~40–60%"). The 0.1 understated short-form integration value ~4–5×.

**Decision:** Set `_SHORTS_FORMAT = 0.5`. Market research (2026 Indian/global creator rate cards) consistently prices dedicated Shorts integrations at 0.4–0.6× of long-form (multiple independent sources: 20–40%, 40–60%, 0.4–0.6×); 0.5 is the midpoint and the defensible value. The docstring and the §14 / ADR-0022 model description are aligned to 0.5. This is a claims-traced change: it re-prices every short-form creator's `est_cost_inr` upward (e.g. the Fitness example 338K median → ~₹118K), bringing them into the same ballpark as comparable long-form, which is the correct outcome.

**Consequences:** Short-form costs now reconcile with reach. Surfaced two coherence fixes shipped alongside: (1) the brand card showed "Avg. views" (mean) while pricing used median — relabeled to "Typical views" (median, the priced figure) so a reviewer dividing cost ÷ views gets a sane CPM; (2) added a derived `is_short` flag to `/match` results + a "Shorts" chip on the card, so short-form pricing is legible. Dead `_est_cost` left in place (out of scope; flagged for later removal). No résumé bullet quotes a specific cost figure, so no bullet number changes — only the model description (Shorts 0.5×) is updated.

## ADR-0035 — cost_basis label scheme + min-video pricing guard
**Status:** Accepted (Day 14)
**Context:** Sponsored-cost figures are only as trustworthy as the reach behind them. Two failure modes: (1) channels with very few uploads have no stable per-video reach, so a single viral upload produces a wildly inflated "typical views" and a nonsense cost; (2) the UI showed a single point estimate with no signal of how it was derived, so a ₹50L-capped figure and a well-supported mid-range figure looked equally confident. The pricing path (`attach_est_earnings` → `integration_cost_range`) already produced a low/high band, a `_CAP = ₹5,000,000` ceiling, and a subscriber floor, but none of that provenance reached the surface.
**Decision:** Introduce a `cost_basis` label on every priced row, serialized on all endpoints (`/creators`, `/match`, detail, chatbot): `base` (single-point, within band), `range` (low≠high band), `cap` (hit the ₹50L ceiling), `insufficient` (<`_MIN_VIDEOS_FOR_PRICING` = 10 uploads → cost suppressed to null, but the midpoint est is retained internally so `budget_fit` ranking isn't poisoned by NaN), `unverified` (null `mean_duration_seconds` → format unknown → cost suppressed, risk retained). Brand-match additionally **excludes** <10-upload channels from candidates entirely: one viral short is not an investable track record for a sponsorship recommendation.
**Consequences:** Cross-surface parity enforced — `cost_basis` is added to every serializer in the same change (a field on one endpoint is silently missing from the others otherwise). The chatbot serializers null the cost and add a `cost_note` when insufficient/unverified. A views/subs heuristic for the unverified case was tested and rejected on evidence (didn't improve on suppression). No résumé bullet quotes a specific cost, so no bullet number changes; the §14 pricing description gains the cost_basis enum.

## ADR-0036 — Deterministic missing-channel reply (anti-hallucination)
**Status:** Accepted (Day 14)
**Context:** When a user asks the product assistant why a named channel isn't in CreatorPulse, an LLM left to answer freely will confidently invent a reason (or worse, invent stats for a channel that doesn't exist). The catalogue is a fixed point-in-time snapshot of ~12,500 channels, so "why isn't X here" has a small set of true answers that don't need generation.
**Decision:** Add `_missing_channel_reply(msgs)` ahead of the Groq call in `/chat`. It fires only when both conditions hold: the message matches `_MISSING_RE` (a "why is/isn't … in/on/present/found/included/listed/available/database/corpus" pattern) **and** a quoted channel name is present (straight or curly quotes). If the quoted name matches an indexed title → deterministic "Good news — {title} is indexed…" pointer (no LLM, no invented stats). If absent → a fixed 3-bullet markdown (not in seed snapshot / filtered at ingestion / channel-ID didn't resolve) plus a "periodic updates planned" line. Any message failing either gate falls through to the normal grounded LLM path.
**Consequences:** The two highest-risk hallucination cases (fabricated absence-reasons, fabricated stats for non-existent channels) are handled by templated text that traces to the real catalogue state. Curly-quote-safe. Both directions verified live via curl against the deployed Space before this ADR. Complements ADR-0025's grounded-assistant bounds.

## ADR-0037 — Torch-free Streamlit frontend via the deployed /match API
**Status:** Accepted (Day 14)
**Context:** The Streamlit persona app is deployed to Streamlit Community Cloud (~1 GB RAM). Its brand page called `apps.ml.match.match()` in-process, which loads sentence-transformers + torch to encode the brief — the single largest memory cost and a real OOM risk on Cloud. The FastAPI product API is already deployed (HF Spaces) against Neon and owns the same match logic, verified live.
**Decision:** Refactor the Streamlit brand match to POST to the deployed `{API_URL}/match` and build the results DataFrame from the JSON, rather than loading the model locally. The frontend becomes torch-free; the Space is the single match service (the Next.js frontend already consumes it — this makes Streamlit consistent). The creator page keeps its direct-Neon reads (no torch). Separately, `staging.videos` (124,527 rows) was never migrated to Neon, so the archetype top-videos query failed in prod; migrated a lean `marts.video_titles` (video_id→title, ~12 MB) lookup and repointed the join to it.
**Consequences:** Streamlit `requirements.txt` drops torch/sentence-transformers → fast Cloud build, no OOM. Still requires `scikit-learn`/`scipy` (to unpickle the KMeans/PCA/StandardScaler cluster bundle) and the Neon `DATABASE_URL` (creator page). The `/match` response is a superset of what the brand page renders, so the redundant `creators_df` merge (which caused a title `_x/_y` suffix collision, ADR-0031) was removed. All three pages incognito-verified on `creatorpulse-india.streamlit.app`; server-side PostHog events confirmed landing from the deployed app.
